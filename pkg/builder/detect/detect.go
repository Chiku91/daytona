...
func DetectProjectBuilderType(project *workspace.Project, projectDir string, sshSessionConfig *ssh.SessionConfig) (BuilderType, error) {
	if project.Build != nil && project.Build.Devcontainer != nil {
		return BuilderTypeDevcontainer, nil
	}

	if sshSessionConfig != nil {
		if _, err := ssh.ReadFile(sshSessionConfig, path.Join(projectDir, ".devcontainer/devcontainer.json")); err == nil {
			project.Build.Devcontainer = &workspace.ProjectBuildDevcontainer{
				DevContainerFilePath: ".devcontainer/devcontainer.json",
			}
			return BuilderTypeDevcontainer, nil
		}
		if _, err := ssh.ReadFile(sshSessionConfig, path.Join(projectDir, ".devcontainer.json")); err == nil {
			project.Build.Devcontainer = &workspace.ProjectBuildDevcontainer{
